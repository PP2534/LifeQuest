<x-app-layout>
    ...
    <form method="GET" action="{{ route('users.index') }}" class="flex flex-wrap md:flex-nowrap gap-3 mb-6">
        <input type="search" name="search" placeholder="Tìm bạn theo tên..." value="{{ request('search') }}" 
         class="border border-gray-300 rounded p-2 w-full md:w-1/4 focus:outline-none focus:ring-2 focus:ring-teal-500">
        <select name="interest" class="border border-gray-300 rounded-md px-4 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-teal-500">
        ...
        <select name="province_id" class="border border-gray-300 rounded-md px-4 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-teal-500">
         ...
         <select name="ward_id" class="border border-gray-300 rounded p-2 w-full md:w-1/4 pr-8 focus:outline-none focus:ring-2 focus:ring-teal-500">
        ...
        <button type="submit" class="border rounded p-2 w-full md:w-32 bg-amber-500 hover:bg-amber-600
    ...
    @empty
        <div class="col-span-1 md:col-span-3 text-center text-gray-500 py-6">
            {{ $errorMessage ?? 'Không tìm thấy bạn bè nào phù hợp.' }}
        </div>
    @endforelse
</section>
