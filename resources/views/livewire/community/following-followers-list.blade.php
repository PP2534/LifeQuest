<div>
    ...
    <nav role="tablist" class="flex space-x-4 border-b border-gray-300 mb-8">
        <button wire:click="$set('activeTab', 'following')"
                id="tab-following"
                class="pb-2 border-b-2 font-semibold focus:outline-none
                       {{ $activeTab == 'following' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-600 hover:text-teal-600' }}">
            Đang theo dõi
        </button>
        <button wire:click="$set('activeTab', 'followers')"
                id="tab-followers"
                class="pb-2 border-b-2 font-semibold focus:outline-none
                       {{ $activeTab == 'followers' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-600 hover:text-teal-600' }}">
            Người theo dõi
        </button>
    </nav>
        ...
        <ul class="space-y-6">
            @forelse($followings as $user)
                <li class="bg-white p-4 rounded-lg shadow flex items-center space-x-4">
                    <img src="{{ $user->avatar ?? 'https://i.pravatar.cc/60?u='.$user->id }}" 
                    ...
                    <button wire:click="unfollow({{ $user->id }})"
                            class="px-4 py-2 rounded-md border border-teal-600 text-teal-600 font-semibold hover:bg-teal-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-teal-400"
                            aria-label="Bỏ theo dõi {{ $user->name }}">
                        Bỏ theo dõi
                    </button>
                </li>
            @empty
                ...
                <li class="bg-white p-4 rounded-lg shadow flex items-center space-x-4">
                    ...
                    @if(!Auth::user()->followingsUsers->contains($user->id))
                        <button wire:click="follow({{ $user->id }})"
                                class="px-4 py-2 rounded-md bg-teal-600 text-white font-semibold hover:bg-teal-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-teal-400"
